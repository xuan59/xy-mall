<template>
  <div>
    ...
    >
      <el-table-column type="expand">
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="登录认证信息">
              <span>{{ props.row.accessToken }}</span>
            </el-form-item>
            <el-form-item width="150px" label="第三方登录id">
              <span>{{ props.row.sourceUid }}</span>
            </el-form-item>
            <el-form-item label="第三方授权码">
              <span>{{ props.row.accessCode }}</span>
            </el-form-item>
          </el-form>
        </template>
      </el-table-column>
      ...
      <el-table-column label="账号状态" prop="status" sortable width="125%" align="center">
...
export default {
  ...
  methods: {
    handleEdit(index, row) {
      this.axios.post('/saveCommonUser', row).then(data => {
    ...
    handleDelete(index, row) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          this.axios.post('/delCommonUser', row).then(data => {
            if (data.data.status === '1') {
              this.$message({
                message: '恭喜你，删除成功',
                type: 'success'
    ...
    delMultipleUser() {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        .then(() => {
          this.axios
            .post('/delMultipleCommonUser', this.multipleSelection)
            .then(data => {
              if (data.data.status === '1') {
  ...
  created() {
    this.axios.post('/getCommonUserList').then(data => {
