<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>退货订单</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    >
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" v-if="scope.row.status == 5"  @click="handleEdit(scope.$index, scope.row)">同意退款</el-button>
          <el-button type="primary" size="mini" v-if="scope.row.status == 6" disabled @click="handleEdit(scope.$index, scope.row)">退款中</el-button>
          <el-button type="primary" size="mini" v-if="scope.row.status == 7" disabled @click="handleEdit(scope.$index, scope.row)">退款成功</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    handleEdit(index, row) {
      ...
      this.axios.post('http://192.168.43.179:8085//updateOrderStatus', param).then(data => {
        if (data.data === 1) {
          ...
          this.$message.error('错了哦，同意退款失败，请重试')
        }
      })
    },
    ...
    multipleOrder() {
      ...
      this.axios
        .post('http://192.168.43.179:8085//updateOrderListStatus', this.multipleSelection)
        .then(data => {
          if (data.data === 1) {
            this.$message({
              message: '同意退款成功',
  ...
  created() {
    this.axios.post('http://192.168.43.179:8085/getOrderListRefund').then(data => {
